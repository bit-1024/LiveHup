...
CREATE TABLE IF NOT EXISTS users (
    ...
    is_new_user TINYINT(1) DEFAULT 1 COMMENT '是否新用户',
...
CREATE TABLE IF NOT EXISTS point_records (
    ...
    INDEX idx_created_at (created_at),
    INDEX idx_user_expire (user_id, is_expired, expire_date)
...
CREATE TABLE IF NOT EXISTS exchanges (
    ...
    INDEX idx_user_status (user_id, status, exchange_date)
...
('admin', '$2a$10$N9qo8uLOickgx2ZMRZoMye1J7TgqK9VQ/L3XZ7K3Z8K3Z8K3Z8K3Z', '系统管理员', 'super_admin', 'admin@example.com');

-- 插入系统配置
INSERT INTO system_config (config_key, config_value, config_type, description, is_public) VALUES 
('site_name', '社群积分系统', 'string', '网站名称', 1),
('points_expire_reminder_days', '7', 'number', '积分过期提醒天数', 0),
('exchange_audit_required', 'true', 'boolean', '兑换是否需要审核', 0),
('max_exchange_per_day', '10', 'number', '每天最大兑换次数', 1),
('min_points_for_exchange', '100', 'number', '最低兑换积分', 1);
...
('品牌水杯', '高档保温水杯,容量500ml', 200, 89.00, 100, '生活用品', 1, 1),
('蓝牙耳机', '无线蓝牙5.0耳机,降噪功能', 500, 199.00, 50, '数码产品', 1, 1),
('定制T恤', '社群专属T恤', 300, 69.00, 200, '服饰', 1, 0),
('电影票', '通用电影票兑换券', 150, 50.00, -1, '娱乐', 1, 0),
('优惠券10元', '全场通用10元优惠券', 50, 10.00, -1, '优惠券', 1, 0);
...
SELECT 
    ...
    SUM(CASE WHEN pr.source = 'import' THEN 1 ELSE 0 END) as import_count,
    SUM(CASE WHEN pr.source = 'exchange' THEN 1 ELSE 0 END) as exchange_count,
    SUM(CASE WHEN pr.points > 0 AND pr.expire_date > CURDATE() THEN pr.points ELSE 0 END) as valid_points,
    SUM(CASE WHEN pr.is_expired = 1 THEN pr.points ELSE 0 END) as expired_points_detail
FROM users u
LEFT JOIN point_records pr ON u.user_id = pr.user_id
GROUP BY u.user_id, u.username, u.total_points, u.available_points, u.used_points, u.expired_points, u.is_new_user, u.first_import_date;
...
GROUP BY p.id, p.name, p.category, p.points_required, p.stock, p.sold_count;

-- ============================================================
-- 创建存储过程 (MySQL 5.6兼容版本)
-- ============================================================

DELIMITER //

DROP PROCEDURE IF EXISTS sp_process_expired_points//
...
-- 用户积分变动后更新统计
DROP TRIGGER IF EXISTS trg_after_point_record_insert//
...
-- 商品兑换后更新库存
DROP TRIGGER IF EXISTS trg_after_exchange_insert//
...
-- 兑换取消后恢复库存和积分
DROP TRIGGER IF EXISTS trg_after_exchange_cancelled//
...
-- ============================================================
-- MySQL 5.6 不支持事件调度器的情况说明
-- ============================================================
-- 注意：MySQL 5.6 支持事件调度器，但可能默认未开启
-- 需要手动开启：SET GLOBAL event_scheduler = ON;
-- 或在 my.cnf 中添加：event_scheduler=ON
...
-- 创建每日凌晨1点处理过期积分的事件
DROP EVENT IF EXISTS evt_daily_expire_points;

CREATE EVENT evt_daily_expire_points
...
-- 完成
-- ============================================================
SELECT 'Database schema for MySQL 5.6 created successfully!' as message;
SELECT 'Please ensure event_scheduler is enabled: SET GLOBAL event_scheduler = ON;' as reminder;